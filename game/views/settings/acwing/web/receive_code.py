...
def receive_code(request):
    ...
    if players.exists(): # 如果该用户已存在，则无需重新获取信息，直接登录即可
        ...
        return redirect(reverse("global_index") + "?access=%s&refresh=%s" % (str(refres.access_token), str(refresh)))
    ...
    return redirect(reverse("global_index") + "?access=%s&refresh=%s" % (str(refresh.access_token), str(refresh)))
